...
# composes into `mp-ml-team` on the serving cluster and can't collide with
# another team's. The prefix keeps a team namespace named `default` or
# `kube-system` off the cluster's own. The prefix and label are a cross-function
# contract with compose-model-route and compose-model-cache.
_NS_PREFIX = "mp-"
...
def remote_namespace(replica: v1alpha1.ModelReplica) -> str:
    ...
    return f"{_NS_PREFIX}{_namespace(replica.metadata)}"
