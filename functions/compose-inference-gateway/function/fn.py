...
"""Compose the InferenceGateway: the front door for inference requests.
...
A cluster hosts at most one InferenceGateway, because a second would contend for
...
# Names of the objects composed onto the gateway's cluster. One InferenceGateway
# per cluster, so these are fixed names rather than derived from the XR's.
_GATEWAY_NAME = "inference-gateway"
_HEALTHZ_NAME = "inference-gateway-healthz"
_CALLERS_NAME = "inference-gateway-callers"
_FAILOVER_NAME = "inference-gateway-failover"
_REDIRECT_NAME = "inference-gateway-redirect"

# The Gateway's listener names. When the gateway terminates TLS, inference binds
# to the HTTPS listener alone (compose-model-route sets the AIGatewayRoute's
# sectionName to match), so a caller's credentials never cross the wire in the
# clear; :80 then carries only /healthz and a redirect to :443.
_LISTENER_HTTP = "http"
_LISTENER_HTTPS = "https"

# The GatewayClass the ServingStack installs. Its XRD defaults className to
# this, and a cluster hosting an InferenceGateway is one the ServingStack has
...
# certificates from this CA, which is how the gateway proves itself and how
# anything else is refused. The CA's private key never leaves this cluster.
...
# compose_client_pki.
_CLIENT_CA_BUNDLE = "inference-gateway-ca"
...
def _url_host(address: str) -> str:
    ...
    A bare IPv6 literal collides with the port separator in a URL, so it has to
    be wrapped in brackets; a hostname or IPv4 literal is used unchanged.
    """
    return f"[{address}]" if _ip_version(address) == _IP_VERSION_6 else address
...
def _k8s_object(provider_config: str, manifest: dict, *, ready_when: str | None = None) -> k8sobjv1alpha1.Object:
    ...
    ready_when query so its readiness reflects having been programmed.
    """
    readiness = (
...
class Composer:
    ...
    def resolve_inputs(self) -> bool:
        ...
        Returns False, having set conditions, only when the cluster can't be
        resolved or is owned by another gateway: without its ClusterProviderConfig
        there is nowhere to compose. A missing Secret doesn't gate here, since
        withdrawing the Gateway would move its address; resolve_secrets and
        derive_conditions handle those.
        """
        ...
        # Every InferenceCluster: this gateway's own, for its ProviderConfig and
        # to settle ownership, and the rest to resolve each cluster gateway's name
        # to its address here (see compose_cluster_names).
        response.require_resources(
            self.rsp,
            name="clusters",
            api_version="modelplane.ai/v1alpha1",
        ...
        if "clusters" not in self.req.required_resources or "gateways" not in self.req.required_resources:
            self.not_ready(
        ...
        for c in request.get_required_resources(self.req, "clusters"):
            ...
            if _name(cluster.metadata) == self.xr.spec.clusterName:
    ...
    def resolve_secrets(self) -> None:
        ...
        A missing Secret is left missing rather than gating the composition. It
        keeps the gateway from reporting ready, reported in derive_conditions,
        but withdrawing the Gateway to wait for a Secret would delete its load
        balancer and move the address callers use, which a gateway must never do.
        A missing TLS Secret leaves the HTTPS listener unprogrammed; missing
        caller keys leave the auth policy denying every request, see
        compose_caller_auth.
        """
        if self.xr.spec.auth:
            self.caller_secrets = request.get_required_resources(self.req, "caller-secrets")
        for i, _ in enumerate(self.xr.spec.tls.certificateRefs if self.xr.spec.tls else []):
            found = request.get_required_resources(self.req, f"tls-secret-{i}")
            if found:
    ...
    def compose_secrets(self) -> None:
        ...
        Keyed by the source Secret's name, not its position in the selector
        results, which the API server doesn't order stably.
        """
    ...
    def compose_gateway(self) -> None:
        ...
        carries TLS, and inference then binds to it alone so credentials never
        cross :80 in the clear; :80 keeps only /healthz and a redirect (see
        compose_redirect).
    ...
    def compose_client_pki(self) -> None:
        ...
        objects: list[tuple[str, dict, str | None]] = [
            ...
            (
                "client-ca-certificate",
                {
                    ...
                    "spec": {
                        ...
                        "commonName": f"Modelplane InferenceGateway CA {gateway}"[:64],
            ...
            # gateway needs this certificate to know an InferenceGateway is
            # calling, and the only route to it is through this gateway's status.
            #
        ...
        for key, manifest, cel in objects:
            obj = _k8s_object(self.pc, manifest, ready_when=cel)
    ...
    def compose_caller_auth(self) -> None:
        ...
        When auth is asked for but no caller Secret has resolved there is nothing
        to match a key against, so the policy denies every request rather than
        leaving the gateway open, and the apiKeyAuth policy replaces it once a
        Secret resolves. Inference routes carry no policy of their own, so this
        gateway-level deny reaches them; /healthz and the :80 redirect carry an
        allow policy each, so a health check and the redirect still answer.
        """
        if not self.xr.spec.auth:
            return
        target = {"group": "gateway.networking.k8s.io", "kind": "Gateway", "name": _GATEWAY_NAME}
        if self.caller_secrets:
            ...
            spec = {"targetRefs": [target], "authorization": {"defaultAction": "Deny"}}
        resource.update(
                ...
                self.pc,
                {
                    ...
                    "spec": {
                        ...
                        "healthCheck": {
                            ...
                            # panicThreshold 0 disables panic mode. By default,
                            # once more than half a cluster's endpoints are
                            # unhealthy Envoy ignores health checks and load
                            # balances over all of them, ejected ones included.
                            # The endpoints behind one ModelService are a single
                            # Envoy cluster (not one per InferenceCluster), so
                            # ejecting a whole priority tier usually crosses that
                            # half and would defeat the failover this policy exists
                            # for. Better to refuse a request than serve it to a
                            # backend known dead.
                            "panicThreshold": 0,
                        },
                    },
    ...
    def compose_redirect(self) -> None:
        """Redirect everything on :80 to HTTPS, except /healthz.

        Only when the gateway terminates TLS. Inference binds to the HTTPS
        listener alone, so without this a caller reaching :80 would just 404;
        the redirect sends it to :443 instead, and /healthz (an Exact match)
        outranks this catch-all so a health check over plain HTTP still answers.

        The redirect has to happen before authentication, or an unauthenticated
        caller would get a 401 rather than being sent to HTTPS. The caller
        SecurityPolicy targets the whole Gateway, so this route carries an
        allow-all policy to override it, the way /healthz does.
        """
        if not self.xr.spec.tls:
            return
        resource.update(
            self.rsp.desired.resources["redirect-route"],
            _k8s_object(
                self.pc,
                {
                    "apiVersion": "gateway.networking.k8s.io/v1",
                    "kind": "HTTPRoute",
                    "metadata": {"name": _REDIRECT_NAME, "namespace": REMOTE_NAMESPACE},
                    "spec": {
                        "parentRefs": [
                            {
                                "group": "gateway.networking.k8s.io",
        ...
        if not self.xr.spec.auth:
            return
        resource.update(
            self.rsp.desired.resources["redirect-auth"],
            _k8s_object(
                self.pc,
                {
                    "apiVersion": "gateway.envoyproxy.io/v1alpha1",
                    "kind": "SecurityPolicy",
                    "metadata": {"name": f"{_REDIRECT_NAME}-open", "namespace": REMOTE_NAMESPACE},
                    "spec": {
                        "targetRefs": [
                            {
                                "group": "gateway.networking.k8s.io",
    ...
    def not_ready(self, reason: str, message: str) -> None:
        ...
        Marks the composite not-ready explicitly. A degraded gateway still
        composes a Gateway, which reports ready once it has an address, so the XR
        would otherwise aggregate to ready while its HTTPS listener has no
        certificate or its caller policy denies everyone.
        """
    ...
    def derive_conditions(self) -> None:
        ...
        A missing input Secret is reported here rather than by withdrawing the
        Gateway, whose address must survive it. The Gateway's own readiness only
        tracks having an address, so neither a fail-closed auth policy nor an
        unprogrammed HTTPS listener would otherwise show.
        """
        if self.xr.spec.auth and not self.caller_secrets:
        ...
        missing_tls = [
            ref.name
            for i, ref in enumerate(self.xr.spec.tls.certificateRefs if self.xr.spec.tls else [])
            if not request.get_required_resources(self.req, f"tls-secret-{i}")
        ]
        if missing_tls:
            self.not_ready(CONDITION_REASON_SECRETS_MISSING, f"Waiting for TLS Secrets: {', '.join(missing_tls)}")
            return
        waiting = [
            key
