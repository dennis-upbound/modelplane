...
class TestFunctionRunner(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_full_gateway(self) -> None:
        ...
        self.assertEqual(
            manifest("healthz-route")["spec"]["parentRefs"],
            [
                {
    ...
    async def test_an_ipv6_address_is_bracketed_in_the_endpoints(self) -> None:
    ...
    async def test_a_missing_caller_secret_denies_but_keeps_the_gateway(self) -> None:
        ...
        for name, extra, message in [
            ...
            with self.subTest(name):
                req = fnv1.RunFunctionRequest(
                    ...
                    required_resources=_required(
                        clusters=[_cluster()], gateways=[_gateway_xr("eu", _CLUSTER)], **extra
                    ),
                )
                got = await self.runner.RunFunction(req, None)

                self.assertIn("gateway", got.desired.resources, "the Gateway is kept, so its address survives")
                self.assertFalse(
                    any(key.startswith("caller-secret-") for key in got.desired.resources),
                    "no caller Secret resolved, so none is copied to the cluster",
                )
                spec = resource.struct_to_dict(got.desired.resources["caller-auth"].resource)["spec"]["forProvider"][
                    "manifest"
                ]["spec"]
                self.assertEqual(
                    spec,
                ...
                cond = next(iter(got.conditions))
                self.assertEqual(cond.status, fnv1.STATUS_CONDITION_FALSE)
                self.assertEqual(cond.reason, fn.CONDITION_REASON_SECRETS_MISSING)
                self.assertEqual(cond.message, message)

    async def test_a_missing_tls_secret_keeps_the_gateway(self) -> None:
        ...
        self.assertIn("gateway", got.desired.resources, "the Gateway is kept, so its address survives")
        self.assertNotIn("tls-secret-eu-tls-0", got.desired.resources, "the missing Secret isn't copied to the cluster")
        listeners = resource.struct_to_dict(got.desired.resources["gateway"].resource)["spec"]["forProvider"][
            "manifest"
        ]["spec"]["listeners"]
        self.assertEqual([ln["name"] for ln in listeners], ["http", "https"], "the HTTPS listener is still declared")
        cond = next(iter(got.conditions))
        self.assertEqual(cond.status, fnv1.STATUS_CONDITION_FALSE)
        self.assertEqual(cond.reason, fn.CONDITION_REASON_SECRETS_MISSING)
        self.assertEqual(cond.message, "Waiting for TLS Secrets: eu-tls-0")
