...
# in namespace `ml-team` composes into `mp-ml-team` on the workload cluster. Must
# match the namespace the serving pods mount from (compose-model-replica's
# base.remote_namespace): a pod can only mount a PVC in its own namespace. The
# two functions set this independently, so they are a contract - change together.
_NS_PREFIX = "mp-"
